...
export const CHANGELOG = [
  {
    ...
    patches: [
      {
        version: 'V1.3.1', date: '2026-09-03',
        items: [
          '帮助中心上线：导航栏问号或设置页进入，使用教程、功能说明、货币计费、会员权益、常见问题、开源版权六大板块一站查齐，手机端也有专属排版',
          '用户交流群开张：设置页、反馈页、首页底部胶囊三处入口，悬浮即可扫码加入，群里有 AI 助手随时答疑',
          '会员权益页补齐「会员协议 · 兑换码开通」入口，有码的朋友不用再绕路',
          '结果页的 SRT、Markdown 笔记旁新增小问号，悬停即可了解这些格式能做什么',
          '修复小红书来源标签显示为网址的问题，现在正确显示平台名称',
          '修复在二级界面中点头像进入设置时，下拉卡片偶尔闪现的问题',
          '修复切换页面后停留在旧滚动位置的问题，现在每次进入页面都回到顶部',
          '细节打磨：货币胶囊点开弹层后悬浮层即时让位；首页彩蛋诗改为每日轮换',
        ],
      },
    ],
  },
  {
    ...
    // 开发者寄语（碳碳亲笔）
    intro: '开发者的悄悄话：小伙伴们好呀！新的大版本我们整合了之前版本的问题，新增了能在线渲染的"文件柜"功能，加固了许多安全侧防护，希望各位星旅者在V1.2.x 用得开心～',
    items: [
      '复制视频链接后回到本站，自动识别并弹出预估确认卡——从复制到开始提取一步直达（Safari 可在页面空白处直接粘贴触发）',
      '星轨实验室上线（设置 → 关于 · 版本日志下方）：还在打磨的新功能会在这里陆续登场，先申请先体验',
      '文件柜（实验室首个内测功能）：提取结果一键转存云端，字幕/全文/笔记/概要/AI 解读随时在线查看渲染版——Markdown 笔记、公式、表格原样呈现，手机也能看；配额可视，文件夹整理，与提取记录互不影响',
      '转存入口融入每个「下载」按钮：悬浮或点击即可选择「下载文件」或「转存到文件柜」，无需学习成本',
      '体验细节：引导弹窗翻页不再忽高忽低、圆点恒居中；删除确认等弹窗全部居中',
      '安全加固：文件柜多层防护（专用密码/配额/权限隔离），预估接口限流，诊断导出再扩充',
    ],
    patches: [
      {
        version: 'V1.2.2', date: '2026-08-27',
        items: [
          '登录/注册的人机验证焕新：换成秒开的图形验证码——不再转圈半天后告诉你验证失败',
          '修复：AI 解读遇到复杂问题时可能永远加载不出答案——现在思考再久也会给你回答；实在答不出会明确告诉你，而不是留你对着转圈发呆',
          '修复：少数情况下 AI 没能回答却仍被误扣引力波的问题——现在回答失败绝不扣费',
          '隐私更干净：人机验证改为本站自行完成，不再向境外第三方传输任何数据',
          '幕后：开发者给 AI 装上了健康监控，异常会更早被发现和修复',
        ],
      },
      {
        version: 'V1.2.1', date: '2026-08-11',
        items: [
          '修复：内测申请的状态此前可能被一条普通的工单回复误标为「未通过」，还可能被误锁 7 天——现在申请状态只跟随真实的审批结果，不会再有误伤',
          '申请进度更可靠：只要你的申请还没有明确结论，它就会一直显示「审核中」并留在开发者的待办列表里，不会再悄悄消失',
          '若申请未通过，页面会清楚告诉你何时可以再次申请；每一次通过与拒绝都会留痕，误操作也能撤回',
        ],
      },
    ],
  },
