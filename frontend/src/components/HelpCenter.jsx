...
历史记录支持搜索。**关键词搜索**免费即时，匹配标题与字幕原文；**AI 语义搜索**按"我记得大意"模糊匹配（比如搜"讲物理思维的"也能找到量纲分析那期），每次固定消耗 \`1\` 引力波，搜索不到不扣费。

### 分享卡片

结果页的概要区块点击「分享」，把 AI 概要渲染成一张排版好的 PNG 分享图。**纯本地生成**，内容不出你的浏览器，不产生任何消耗。

### 版本星图（限定）

「设置 → 关于 → 版本星图」：60 个版本连成的星空，纪念 Stellaris 的第 60 个版本。滚轮缩放、拖拽平移、点击星点开启版本巡游。该功能为纪念限定，后续是否长期保留视运营情况决定。
`
...
驱动 Markdown 结构化笔记、AI 解读、AI 语义搜索。汇率：\`1 引力波 = 500 tokens\`。

注册即送 \`30\` 个，**永不过期**；注册礼、会员月赠、兑换、活动全部进入同一个永久钱包。

> 例：16 分钟视频生成 MD 笔记约消耗 5000 tokens，结算为 10 引力波；AI 解读每轮约 1500 tokens，结算为 3 引力波；AI 语义搜索每次固定 500 tokens，结算为 1 引力波。
