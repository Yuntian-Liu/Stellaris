...
async def download_result(task_id: str, format: str):
    ...
    if format not in ("srt", "txt"):
        raise HTTPException(status_code=400, detail="格式仅支持 srt 或 txt")
    ...
    if not file_path.exists():
        raise HTTPException(status_code=404, detail="文件不存在（任务可能尚未完成）")

    media_type = "text/plain" if format == "txt" else "application/x-subrip"
    return FileResponse(
        path=str(file_path),
...
async def run_pipeline(
    ...
    try:
        ...
        if not cc_segments:
            ...
            asr_result = await transcribe_with_mimo(audio_path, task_id)
        ...
        export_paths = save_exports(task_id, srt_content, txt_content)

        # ✅ 完成
        _update_status(task_id, TaskStatus.COMPLETED, 100, extra={
            "video_title": video_title,
            "subtitle_srt": str(export_paths["srt_path"]),
            "subtitle_txt": str(export_paths["txt_path"]),
...
async def run_pipeline_from_file(
    ...
    try:
        ...
        srt_content = segments_to_srt(segments)
        txt_content = segments_to_txt(segments)
        save_exports(task_id, srt_content, txt_content)

        _update_status(task_id, TaskStatus.COMPLETED, 100, extra={
            "video_title": video_title,
            "subtitle_srt": "output.srt",
            "subtitle_txt": "output.txt",
